/**
 * 构建 data/locations.js — 111 点位 + 459 旅游攻略 + 1-3 星体系
 *
 * 数据源：
...
function parseSlices(lines) {
  ...
  while (i < lines.length) {
    ...
    while (i < lines.length && !/^【(标杆精细版|模板生成版)】/.test(lines[i].trim())) {
      ...
      } else if (t.startsWith('旅游特色：')) slice.edu = t.slice('旅游特色：'.length).trim();
...
const TYPE_ISSUES = {
  ...
  '综合文化场馆':   ['对历史/文化兴趣不足、知识面窄', '信息筛选能力弱、缺乏深度探索习惯'],
  '红色纪念场馆':   ['爱国意识薄弱、不懂感恩、缺乏理想', '责任感差、缺乏勇气与担当'],
  '城市书房':       ['不爱阅读、阅读习惯差', '注意力不集中、缺乏安静专注能力'],
  '青少年活动中心': ['缺乏兴趣爱好、业余生活单一', '不愿与人合作、社交退缩'],
  '群众文化馆':     ['不敢当众表达、羞怯内向', '缺乏艺术兴趣、审美感知弱'],
  '社区成长空间':   ['情绪管理差、易怒敏感', '缺乏规则意识、社交技能不足'],
  '公共图书馆':     ['不爱阅读、探索兴趣低', '坐不住、静不下来'],
...
const TYPE_CO = {
  ...
  '城市书房':       '智育：阅读素养、深度探索、信息检索。 心育：专注力、情绪涵养。 德育：终身探索、自我修养。',
  ...
  '公共图书馆':     '智育：阅读能力、深度探索、信息素养。 心育：专注冥想、情绪调节。 劳育：自主管理、行为自律。',
  '科技科普场馆':   '智育：科学思维、探究精神、创新创造。 劳育：动手实践、问题解决。 心育：挫折耐受、求真实学。',
  '综合实践基地':   '劳育：劳动技能、动手实践、责任担当。 智育：项目探索、问题解决。 交育：协作担当、协商解决。',
...
const NEW_POINTS = [
  ...
  { n: '包头市中小学综合实践旅游中心', d: '九原区', a: '内蒙古自治区包头市九原区职教园区思源道', h: '待核实（不对外散客开放）', p: '0472-5881747', lat: 40.651364, lng: 109.993261, type: '综合实践基地' },
...
const fileHeader = `/**
 * 地点数据 - 包头市德育心理建设路径
 * 共 ${out.length} 个点位，${totalSlices} 个旅游攻略

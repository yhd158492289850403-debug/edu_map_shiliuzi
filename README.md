...
> 包头市 111 个旅游景点 × 459 个旅游攻略 × 116 种儿童行为 → 子素养 → 对症攻略
> 微信小程序 + 微信云开发
...
家庭用户输入孩子的一个**行为问题**（如"顶嘴""磨蹭""怕黑"），系统：
1. 匹配 **116 种行为映射表**，找到对应的 1-3 项**子素养**（如"情绪调节""协商与冲突解决"）
2. 在 **111 个包头本地旅游景点**的 **459 个切片攻略**中，找出与该子素养重叠的**对症攻略**
3. 每个攻略含**四段式行程**：行前准备 / 路上话术 / 到馆做法 / 返程复盘
4. 家庭用户可**打卡记录**成长轨迹，生成成长档案
...
- 推荐活动
- 预期发展素养
- 旅游攻略列表（每个切片展示：标题/位置/子素养/适龄/星级/旅游特色/四段攻略）

**切片操作**：
- 点击切片卡片 → 展开四段攻略（行前准备/路上话术/到馆做法/返程复盘）
...
**分享**：支持转发给朋友 + 分享到朋友圈（含行为名 + 子素养 + 攻略标题）。
...
│   ├── detail/               # 详情页：点位完整信息 + 切片攻略 + 打卡入口
│   ├── near/                 # 附近页：定位 → 距离排序 → 半径筛选 → 导航
│   ├── recommend/            # 行为导引页：行为 → 子素养 → 对症攻略推荐
│   ├── profile/              # 个人档案页：打卡统计、成长轨迹、六维分布、身份切换
│   ├── checkin/              # 打卡页：选点位 + 选行为 + 评分 + 笔记 + 照片
│   ├── role-select/          # 身份选择页：家庭用户/成员/领队
│   ├── report/               # 成长报告页：六维雷达图 + 分数 + 建议
│   ├── pdf-export/           # PDF导出页：选择打卡记录 → 导出PDF
│   └── student-detail/       # 成员详情页：领队查看成员信息
│
├── components/               # ===== 5 个自定义组件 =====
│   ├── filter-sheet/         # 筛选面板：维度 / 问题 / 分类 / 星级 / 难度
...
│   ├── getClassStudents/     # 查询：获取团队成员数据（领队用）
...
| `智素` | 认知素养 | 绿 `#22C55E` | 科学思维、探索能力、信息素养 |
...
| `getFilteredLocations(locations, filter)` | 综合筛选：搜索 + 维度 + 星级 + 问题 + 分类 + 专题 + 难度排序 |
...
| `recommend(text, opts)` | 完整推荐：行为 → 子素养 → 对症切片攻略 |
...
| 函数 | 作用 |
|------|------|
| `generateReport(role, scores, behaviors, options)` | 生成报告（家庭用户/成员/领队） |
...
| `getClassStudents` | 领队档案页 | 获取团队成员数据 |
...
3. **六维 1-3 星体系**：星级为定性映射（★★★ 核心 / ★★ 显著 / ★ 一般），非精确评分，基于点位类型 + 旅游特色推断。
4. **四段攻略**：每个切片含 `pre`（行前）/ `talk`（路上）/ `act`（到馆）/ `post`（返程）四段式行程设计。
